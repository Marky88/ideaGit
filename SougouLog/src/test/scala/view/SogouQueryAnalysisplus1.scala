...
object SogouQueryAnalysis {
	...
	def main(args: Array[String]): Unit = {
		...
		// step2、加载数据，封装到RDD
		val rawRDD: RDD[String] = spark.sparkContext.textFile("datas/sogou/SogouQ.reduced", minPartitions = 3)
	 ...
	 * 需求二、用户搜索次数统计
	 */
	def reportSearch(dataframe: DataFrame): Unit = {
		
		val totalDF: Dataset[Row] = dataframe
			// 先按照用户分组，再按照搜索词分组
    		.groupBy(
			    col("user_id"), col("query_words")
		    )
			// 统计各个搜索词被查询点击次数
    		.agg(
			    count(col("query_words")).as("total")
		    )
			// 降序排序
		...
		val aggDF: DataFrame = totalDF.agg(
			max(col("total")).as("max_click_count"), //
			min(col("total")).as("min_click_count"), //
			round(avg(col("total")), 0).as("avg_click_count") //
		)
		aggDF.printSchema()
	 ...
	 * 需求三、搜索时间段统计, 按照每个小时统计用户搜索次数
	 */
	def reportTime(dataframe: DataFrame): Unit = {
		
		val hourDF: Dataset[Row] = dataframe
			// 提取时间字段中小时hour
    		.select(
			    substring(col("query_time"), 3, 2).as("hour")
		    )
			// 按照hour分组和统计
